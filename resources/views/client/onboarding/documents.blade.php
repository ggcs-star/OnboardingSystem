<x-client-layout title="Onboarding">
    <div>
        <h1 class="text-2xl font-semibold text-secondary-dark">Onboarding — {{ $project->product->name }} ({{ $project->brand_name ?? $project->project_name }})</h1>
        <p class="mt-1 text-sm text-secondary">Upload the documents below to finish onboarding {{ $project->product->name }} for {{ $project->brand_name ?? $project->project_name }}.</p>
    </div>
    ...
    @if (session('error'))
        <div class="mt-8 flex items-start gap-3 rounded-xl border border-danger/20 bg-danger-light px-4 py-3 text-sm text-danger">
            <x-icon name="alert" class="mt-0.5 w-4 h-4 shrink-0" />
    ...
    <div class="mt-6">
        @include('client.projects.tabs.documents', ['project' => $project, 'onboarding' => true])
    </div>
</x-client-layout>
