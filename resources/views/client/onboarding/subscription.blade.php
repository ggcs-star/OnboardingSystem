...
<x-client-layout title="Onboarding">
    ...
    @if ($salesEmployees->isNotEmpty())
        <div class="mt-10">
            <h2 class="text-sm font-semibold uppercase tracking-wide text-secondary">Purchased Through</h2>
            <p class="mt-1 text-sm text-secondary">Let us know which salesperson you bought {{ $project->product->name }}</p>
            ...
            @if ($project->sales_employee_id)
                <form method="POST" action="{{ route('client.onboarding.subscription.sales-employee', $project) }}" class="mt-3">
                    @csrf
