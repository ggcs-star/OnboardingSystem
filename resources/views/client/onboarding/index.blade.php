            ...
            <div class="mt-16 border-t border-app-border pt-12">
                ...
                <div class="grid grid-cols-1 gap-6 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4">
                    @foreach($otherProducts as $product)
                        <div class="flex h-full flex-col rounded-2xl border border-app-border bg-white p-6 shadow-sm transition-all duration-300 hover:shadow-md">
                            <div class="mb-4 flex-1">
                                <h3 class="text-lg font-semibold text-secondary-dark">{{ $product->name }}</h3>
                                @if ($product->tagline)
                                    <p class="mt-2 text-sm leading-relaxed text-secondary">{{ $product->tagline }}</p>
                                @endif
                            </div>

                            <!-- Styled Alpine Trigger Button -->
                            <button 
                                type="button" 
                                x-on:click="selectedProduct = {{ $product->id }}; open = true;"
                                class="mt-auto inline-flex w-full items-center justify-center gap-2 rounded-xl border border-gray-300 bg-white px-4 py-2.5 text-sm font-semibold text-gray-700 shadow-sm transition-all hover:bg-gray-50 hover:text-gray-900 focus:outline-none focus:ring-2 focus:ring-gray-900 focus:ring-offset-2"
                            >
