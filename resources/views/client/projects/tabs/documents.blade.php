...
<div x-data="{ search: '', group: '', status: '' }">
    ...
    <div class="mt-4 space-y-6">
        @forelse ($groupedEntries as $groupSlug => $entries)
            @php
                $groupLabel = $entries->first()->group_label;
                $groupMandatory = (bool) $entries->first()->group_mandatory;
            @endphp
            <div class="overflow-hidden rounded-xl border border-app-border bg-white" x-show="group === '' || group === '{{ $groupSlug }}'">
                <form method="POST" action="{{ route('client.projects.documents.bulk-update', $project) }}" enctype="multipart/form-data">
                    ...
                    <div class="flex items-center justify-between gap-3 border-b border-app-border bg-surface-alt px-4 py-3">
                        <h3 class="text-xs font-semibold uppercase tracking-wide text-secondary-dark">{{ $groupLabel }}</h3>
                        @if ($groupMandatory)
                    ...
                    @if (! $readOnly)
                        <div class="flex items-center justify-end gap-3 border-t border-app-border px-4 py-3">
                            @if ($groupMandatory)
                                <p class="text-xs text-secondary">Fields marked <span class="font-medium text-danger">(Mandatory)</span> must be filled to complete onboarding.</p>
                            @endif
                            <x-primary-button class="shrink-0">
                                Submit {{ $groupLabel }} Documents
                            </x-primary-button>
                        </div>
                    @endif
                </form>
        ...
        @empty
            <div class="rounded-xl border border-app-border bg-white px-4 py-10 text-center text-sm text-secondary">
