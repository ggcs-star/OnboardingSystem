...
{{-- Defined as a plain (non-module) script so it runs during HTML parsing,
     before app.js's module script calls Alpine.start() — course-player.js
     (a module, loaded further down) always runs after that point and talks
     to this component only via window CustomEvents, never direct calls. --}}
<script>
...
<div id="course-player" class="mt-6 grid grid-cols-1 gap-6 lg:grid-cols-12" x-data="courseCurriculumPlayer(window.coursePlayerData)" x-init="init()"
        ...
        <template x-if="activeItem && activeItem.type === 'module_quiz'">
            <div class="mx-auto max-w-3xl">
                ...
                <div class="mt-5 space-y-5">
                    @include('client.courses._quiz-questions', ['questionsExpr' => 'activeItem.questions'])
                </div>
    ...
    <div x-show="modalOpen" x-cloak class="fixed inset-0 z-50 flex items-center justify-center bg-black/50 p-4">
        <div class="w-full max-w-lg rounded-xl bg-white p-6 shadow-xl">
            ...
            <div class="mt-5 space-y-5">
                @include('client.courses._quiz-questions', ['questionsExpr' => '(activeCheckpoint?.questions || [])'])
            </div>
...
@vite(['resources/js/course-player.js'])
