...
<div class="max-w-2xl rounded-xl border border-app-border bg-white p-6">
    <form method="POST" action="{{ route('admin.courses.update', $course) }}" enctype="multipart/form-data" class="space-y-5">
        ...
        <div>
            ...
            <select id="product_id" name="product_id" class="mt-1.5 w-full rounded-lg border-app-border text-sm shadow-sm focus:border-primary focus:ring-primary" required>
                @foreach ($products as $product)
                    <option value="{{ $product->id }}" @selected(old('product_id', $course->product_id) == $product->id)>{{ $product->name }}</option>
                @endforeach
            </select>
        ...
        <div>
            ...
            <x-text-input id="title" name="title" class="mt-1.5" :value="old('title', $course->title)" required />
