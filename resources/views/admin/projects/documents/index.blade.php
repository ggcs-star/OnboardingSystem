        ...
        <div class="flex items-start gap-4">
            ...
            <div>
                <div class="flex items-center gap-2">
                    <h1 class="text-xl font-semibold text-secondary-dark">All Documents</h1>
                    @if ($readOnly)
    ...
    <div class="mt-6 grid grid-cols-1 gap-6 xl:grid-cols-4">
        <div class="xl:col-span-3">
            <form method="GET" class="flex flex-col gap-3 sm:flex-row sm:flex-wrap">
                ...
                <select name="group" onchange="this.form.submit()" class="rounded-lg border-app-border text-sm shadow-sm focus:border-primary focus:ring-primary">
                    <option value="">All Groups</option>
                    @foreach ($groups as $slug => $label)
                        <option value="{{ $slug }}" @selected(request('group') === $slug)>{{ $label }}</option>
                    @endforeach
                </select>
                ...
                <a href="{{ route('admin.projects.documents.index', ['project' => $project, 'readonly' => $readOnly ? 1 : null]) }}"
                ...
                <table class="min-w-full divide-y divide-app-border text-sm">
                    ...
                    <tbody class="divide-y divide-app-border">
                        @forelse ($entries as $entry)
                            ...
                            <tr>
                                <td class="px-4 py-3">
                                    <div class="flex items-start gap-2 font-medium text-secondary-dark">
                                        <x-icon name="file-text" class="mt-0.5 w-4 h-4 shrink-0 text-secondary/60" />
                                ...
                                <td class="px-4 py-3">
                                    <a href="{{ route('admin.projects.documents.show', ['project' => $project, 'group' => $entry->group_slug, 'readonly' => $readOnly ? 1 : null]) }}" class="text-secondary hover:text-primary hover:underline">
                                        {{ $entry->group_label }}
                                    </a>
                                </td>
                        ...
                        @empty
                            <tr>
                                <td colspan="6" class="px-4 py-10 text-center text-sm text-secondary">No documents found.</td>
                            </tr>
                        @endforelse
                    </tbody>
                </table>
        ...
        <div class="space-y-6">
            <div class="rounded-xl border border-app-border bg-white p-6">
                <h2 class="text-xs font-semibold uppercase tracking-wide text-secondary">Documents Summary</h2>
                <p class="mt-2 text-sm font-medium text-secondary-dark">All Groups</p>
            ...
            <div class="rounded-xl border border-app-border bg-white p-6">
                <h2 class="text-xs font-semibold uppercase tracking-wide text-secondary">Project Information</h2>
