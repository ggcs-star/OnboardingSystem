...
>
    <div class="flex items-center justify-between px-4 py-5">
        <a href="{{ route('admin.dashboard') }}" class="flex min-w-0 items-center overflow-hidden rounded-lg bg-white p-1.5 shadow-sm" :class="collapsed && 'justify-center'">
            <img x-show="!collapsed" x-cloak src="{{ asset('assets/images/logo.webp') }}" alt="{{ config('app.name', 'Admin') }}" class="h-8 w-auto max-w-[165px] shrink-0 object-contain">
            <img x-show="collapsed" x-cloak src="{{ asset('assets/images/logo-mark.png') }}" alt="{{ config('app.name', 'Admin') }}" class="h-9 w-9 shrink-0 rounded object-cover">
        </a>
    ...
    <nav class="flex-1 space-y-6 overflow-y-auto px-3 pb-6">
        ...
        <div>
            <p x-show="!collapsed" x-cloak class="mb-2 px-3 text-sm font-bold uppercase tracking-wide text-sidebar-text/70">Operations</p>
            <div class="space-y-1">
